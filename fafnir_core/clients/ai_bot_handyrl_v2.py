# fafnir_core/clients/ai_bot_handyrl_v2.py
import asyncio
import argparse
import random
import sys
...
cfg = {"room": "room1", "name": "AI_HandyRL_V2", "url": "http://127.0.0.1:8765"}
...
def select_action_with_search(st: Dict[str, Any], my_idx: int, think_time: float) -> List[str]:
    ...
    while time.time() - start_time < think_time:
        ...
        if not opp_legal:
            ...
            opp_action_id = np.argmax(opp_masked)

        # 3. Simulate all my legal actions in parallel (batched inference)
        sim_envs = []
    ...
    print(f"[AI] Search stats: simulations={runs}, best_val={best_val:.4f}, choice={best_action_tuple}")
...
async def main():
    global AUTO_NEXT, model, THINK_TIME

    ap = argparse.ArgumentParser()
    ap.add_argument("--url", default="http://127.0.0.1:8765")
    ap.add_argument("--room", default="room1")
    ap.add_argument("--name", default="AI_HandyRL_V2")
